...
function appendJournal(ev: JournalEvent, stream: WriteStream): void {
  ...
  const bits = [
    ...
    ev.mid != null ? `mid=${ev.mid.toFixed(3)}` : null,
    ev.centerMode != null ? `ctr=${ev.centerMode}` : null,
    ev.fairValue != null ? `fv=${ev.fairValue.toFixed(3)}` : null,
    ev.edgeCents != null ? `edge=${ev.edgeCents.toFixed(1)}¢` : null,
    ev.inventoryBefore != null ? `inv0=${ev.inventoryBefore}` : null,
    ...
    ev.cashAfter != null ? `cash=${ev.cashAfter.toFixed(2)}` : null,
    ev.reason,
  ].filter(Boolean)
  log(...bits)
}
...
async function runPaperMmLoop(): Promise<void> {
  ...
  portfolio.setConfig({
    ...
    strictRealism: true,
    fvQuoting: true,
    quotingEnabled: true, // U3.2 house mid
