...
| **S5.2** | `L2_OFF_EVICT` (U2.14 / U2.14.1) | Free active slot | `useLiveBook && !liveBook` for `l2OffDropTicks` (default **10**) consecutive `syncMarketUniverse` passes **and** flat inventory → **evict** + refill from ranked open set. If inventory ≠ 0 → hold with fail-loud `U2.14: L2 off — holding inv until flat` (never invent flatten without L2). |
