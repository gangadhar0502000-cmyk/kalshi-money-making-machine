...
export function TradeIdeaCard({ opp, bankroll, onPaperTrade }: Props) {
  ...
  return (
    <article className="panel flex flex-col gap-3 p-4 transition hover:border-slate-600">
      <div className="flex items-start justify-between gap-3">
        ...
        <ScoreBadge score={opp.edgeScore} />
      </div>

      <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
        <Stat label="YES" value={`${formatCents(opp.yesBid)} / ${formatCents(opp.yesAsk)}`} />
        <Stat label="NO" value={`${formatCents(opp.noBid)} / ${formatCents(opp.noAsk)}`} />
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        <span className="rounded-lg bg-slate-800 px-2.5 py-1 text-slate-300">
          Stake ~{formatPct(opp.suggestedStakePct)} ≈ ${stake.toFixed(2)}
        </span>
        <span className="text-xs text-slate-500">
          ~{days < 1 ? `${Math.round(opp.hoursToExpiry)}h` : `${days.toFixed(1)}d`} to close
        </span>
      </div>
      ...
      <div className="mt-auto flex flex-wrap gap-2 pt-1">
        <button type="button" className="btn btn-primary" onClick={() => onPaperTrade(opp)}>
          Paper trade
        </button>
        <a
